...
export const LEVELS: Level[] = [
  ...
  },
  {
    ...
    description: dedent`
      ...
      例えば \`A → B\`、\`B → C\` のようにルールを組み合わせると、間接的な変換が可能になります。
  ...
  },
  {
    ...
    title: "1つ飛ばし",
    description: dedent`
      小文字と数字が交互に並んでいます（例：\`a1b2c3\`）。

      数字はすべて取り除き、小文字は大文字に変換してください（例：\`ABC\`）。
    `,
    m: 2,
    testCases: [
      { input: "z9", target: "Z" },
  ...
  },
  {
    ...
    description: dedent`
      ...
      この 2 進数に $1$ を加えた値を出力してください（例：\`110\` $= 6$）。
