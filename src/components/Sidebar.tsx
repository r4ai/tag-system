...
export function Sidebar({
  ...
  return (
    <>
      ...
      <div
        className={`fixed lg:sticky top-0 inset-y-0 left-0 z-50 w-64 h-screen border-r border-zinc-800 flex flex-col bg-zinc-950/95 backdrop-blur-md transform transition-transform duration-300 ease-in-out ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-6">
          {sections.map((section, sectionIndex) => (
            <div key={section} className="space-y-1">
              ...
              <h3 className="text-xs font-bold text-zinc-500 uppercase tracking-wider px-3 mb-2">
                {section}
              </h3>
              {LEVELS.filter((l) => l.section === section).map((level) => (
                ...
                >
                  ...
                  <ChevronRight
                    className={`w-4 h-4 shrink-0 transition-transform ${currentLevelId === level.id ? "translate-x-1 text-indigo-400" : "opacity-0 group-hover:opacity-100"}`}
                  />
                </button>
              ))}
            </div>
